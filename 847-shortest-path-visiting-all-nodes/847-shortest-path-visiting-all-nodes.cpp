...
public:
    ...
    int helper(vector<vector<int>>& graph) {
        ...
        int endMask = (1 << totalNodes) - 1; // 1111, the node at when we know that we have covered all the nodes.
        
        queue<pair<int, pair<int, int>>> q; // holds three values, value of node, dist we took and bitmask.
        
        set<pair<int, int>> vis; //visited to check we have certain node again.
        ...
        while(!q.empty()) { //until queue does not get empty we will visit all the nodes
            ...
            for(auto temp : graph[val]) { //now will be visiting all the child nodes of the val.
                int newMask = (mask | (1 << temp)); /* now at starting if we have node 0
                then the mask for 0 -> 0001 and now I come to node 1 -> 0010, then we have
                newMask as 0001 | 0010 = 0011*/
                if(newMask == endMask) return dist + 1; //if we are at end node;
                else if(vis.count({temp, newMask})) continue; //if we already visisted a node before with same mask.
